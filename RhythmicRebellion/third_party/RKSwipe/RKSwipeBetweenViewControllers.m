...
CGFloat Y_BUFFER = 13; //%%% number of pixels on top of the segment
...
CGFloat SELECTOR_HEIGHT = 2; //%%% thickness of the selector bar

CGFloat X_OFFSET = 0; //%%% for some reason there's a little bit of a glitchy offset.  I'm going to look for a better workaround in the future
...
- (void)viewDidLoad
{
    ...
    self.navigationBar.barTintColor = [UIColor colorWithRed:0.17 green:0.18 blue:0.36 alpha:1]; //%%% bartint
...
-(void)setupSelector {
    selectionBar = [[UIView alloc]initWithFrame:
                    CGRectMake(X_BUFFER-X_OFFSET,
                               SELECTOR_Y_BUFFER,
                               (self.view.frame.size.width-2*X_BUFFER)/[viewControllerArray count],
                               SELECTOR_HEIGHT)];
    selectionBar.backgroundColor = [UIColor colorWithRed:1 green:0.24 blue:0.65 alpha:1];
