//
//  User.swift
//  RhythmicRebellion
//
//  Created by Alexander Obolentsev on 6/29/18.
//  Copyright © 2018 Patron Empowerment, LLC. All rights reserved.
//

import Foundation

//"data": {
//    "_id": "5b36478e9caba4003a6a3252",
//    "ws_token": "5ce24a949e95cd5cc2356e51d0691209d7de08c4f68b4b35ff8d4d414157e968",
//    "updated_at": "2018-06-29T14:51:58+0000",
//    "created_at": "2018-06-29T14:51:58+0000",
//    "login_token": "eyJ0eXAiOiJKV1QiLCJhbGciOiJIUzI1NiJ9.._NFNWImTpPNeaZe2qmBsxKrYM9XM8ClS869P2fWCKuY",
//    "guest": true
//}

struct User: Decodable {
    let id: String
    let wsToken: String
    let loginToken: String
    let isGuest: Bool

    enum CodingKeys: String, CodingKey {
        case data
    }

    enum DataCodingKeys: String, CodingKey {
        case id = "_id"
        case wsToken = "ws_token"
        case loginToken = "login_token"
        case isGuest = "guest"
    }

     init(from decoder: Decoder) throws {

        let container = try decoder.container(keyedBy: CodingKeys.self)
        let dataCotainer = try container.nestedContainer(keyedBy: DataCodingKeys.self, forKey: .data)

        self.id = try dataCotainer.decode(String.self, forKey: .id)
        self.wsToken = try dataCotainer.decode(String.self, forKey: .wsToken)
        self.loginToken = try dataCotainer.decode(String.self, forKey: .loginToken)
        self.isGuest = try dataCotainer.decode(Bool.self, forKey: .isGuest)
    }

}




