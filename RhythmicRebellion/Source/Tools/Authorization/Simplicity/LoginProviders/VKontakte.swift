...
public class VKontakte: OAuth2 {
    ...
    public override func linkHandler(_ url: URL, callback: @escaping ExternalLoginCallback) {
        
        super.linkHandler(url) { (maybeToken, maybeError) in
            
            if let t = maybeToken, let n = url.fragmentDictionary["email"] as? String {
            ...
            callback(maybeToken, maybeError)
        }
